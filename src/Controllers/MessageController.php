...
class MessageController {
    ...
    private function handleProductSelection($customerId, $message, $lang) {
        ...
        if (preg_match('/^\d+$/', trim($message))) {
            ...
            // Send product image if available (includes confirmation prompt in caption)
            if (!empty($selectedProduct['image_url'])) {
                try {
                    ...
                    // Image caption already includes confirmation prompt, no need for separate message
                    return null;
                } catch (Exception $e) {
                    logMessage("Error sending product image: " . $e->getMessage(), 'ERROR');
                    // If image fails, fall through to text confirmation
                }
            }

            logMessage("After image sending block", 'DEBUG');

            // Only send text confirmation if no image was sent
     ...
     * Directly creates order with quantity = 1 (no quantity prompt)
     */
    private function handleProductConfirmation($customerId, $message, $lang) {
        ...
        if ($messageTrimmed === '1') {
            ...
            // Default quantity = 1 (skip quantity prompt)
            $quantity = 1;
    ...
    /**
     * Send product image with details and confirmation prompt to customer
     */
